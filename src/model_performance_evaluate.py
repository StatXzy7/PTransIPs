...
if __name__ == "__main__":
    ...
    if args.path is None:
        if hasattr(cf, 'ST') and cf.ST:
            args.path = './model/ST_train'
        elif hasattr(cf, 'Y') and cf.Y:
            args.path = './model/Y_train'
        else:
            raise ValueError("Please input the path of model to evaluate")

    if cf.ST:
        train,test = data_read()
        train_seq = train.iloc[:,1]
        test_seq = test.iloc[:,1]
        test_label = torch.tensor(np.array(test.iloc[:,0],dtype='int64')).to(device, non_blocking=True) # Very important
        ...
        test_embedding = torch.tensor(np.load('./data/ST_test_embedding.npy')).to(device, non_blocking=True)
        test_str_embedding = torch.tensor(np.load('./data/ST_test_str_embedding.npy')).to(device, non_blocking=True)
        path = args.path
        ...
        train_encoding = BERT_encoding(train_seq,test_seq).to(device, non_blocking=True)
        test_encoding = BERT_encoding(test_seq,train_seq).to(device, non_blocking=True)
        test_embedding = torch.tensor(np.load('./data/Y_test_embedding.npy')).to(device, non_blocking=True)
        test_str_embedding = torch.tensor(np.load('./data/Y_test_str_embedding.npy')).to(device, non_blocking=True)
        path = args.path

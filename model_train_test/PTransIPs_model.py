...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):
        ...
        enc_outputs = self.dropout(enc_outputs)  # dropout
        return enc_outputs
...
class BERT(nn.Module):
    def __init__(self, config):
        ...
        self.resconv1d = nn.Sequential(
            ResNetConv1d(in_channels=d_model, out_channels=d_model, kernel_size=5, stride=1, padding=2),
            nn.Dropout(0.5),
            nn.ReLU()
        )
    ...
    def forward(self, input_ids, x_embedding, str_embedding):
        ...
        str_embedding = str_embedding.to(torch.float32) 
        # str_embedding = str_embedding[:, :, 33:66].to(torch.float32) 
